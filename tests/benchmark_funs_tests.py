...
def train_test(test_fun, n_dim, forecaster_class=LatentStructuredPICNN, **forecaster_kwargs):
    ...
    ub = np.ones(n_dim)
    lb = -np.ones(n_dim)
    data = make_dataset(test_fun(ndim=n_dim).evaluate, lb, ub, n_sample_per_dim=600)
    ...
    sol = m.optimize(data[x_names].iloc[[0], :], objective=objective_fun,
                     n_iter=10, recompile_obj=False, rel_tol=-1)
    ...
    x_opt, obj_opt, ctrl_coords, obj_opts = [], [],[], []

    for i in np.random.choice(data.shape[0], 6):
        sol = m.optimize(data[x_names].iloc[[i],:], objective=objective_fun, n_iter=12000, recompile_obj=False, rel_tol=-1e14)
        print(sol[0], sol[2].values.ravel(), sol[3])
        ctrl_coords.append(m.input_scaler.inverse_transform(d(np.ones((sol[-2].shape[0], 1)), sol[-2])))
        obj_opts.append(m.predict(pd.DataFrame(np.hstack([np.ones((sol[-2].shape[0], 1)), ctrl_coords[-1]]), columns=x_names)).values.ravel())
        x_opt.append(sol[0])
        obj_opt.append(sol[2].values.ravel())
    x_opt = np.vstack(x_opt)
    obj_opt = np.vstack(obj_opt)
    ...
    if n_dim == 2:
        ...
        fig, ax = plot_surface(pd.concat([data[optimization_vars], y_hat], axis=1), scatter=True, alpha=0.05)
        ax.scatter(*np.hstack([x_opt, obj_opt]).T, c='r', s=1000, marker='*')

        #ax.plot(*ctrl_coords.T, sol[-1].ravel(), c='green', marker='s', markersize=2, alpha=0.1)
        [ax.scatter(*ctrl_i.T, obj_i, c=np.linspace(0, 1, len(obj_i)),s=1, alpha=0.2) for ctrl_i, obj_i in zip(ctrl_coords, obj_opts)]
...
def test1d(forecaster_class, **forecaster_kwargs):
    ...
    # fun from latent reconstruction tr points only
    ax.scatter(*E_tr.T, Z_oracle_tr, c='b', s=50, marker='+', alpha=1)


    for i in np.random.choice(data.shape[0], 5):
        sol = m.optimize(data[['const','x']].iloc[[i],:], objective=lambda x, ctrl: jnp.mean(x), n_iter=5000, recompile_obj=False, rel_tol=-10)
        ax.plot(*sol[-2].T, sol[-1].ravel(), c='green', marker='s', markersize=2, alpha=0.1)
...
if __name__ == '__main__':
    ...
    augment_ctrl_inputs = False, n_hidden_x = 3, n_latent=200, n_out = 1, batch_size = 2500,
    n_epochs = 10, stats_step = 20, n_layers = 3, n_encoder_layers = 3, n_decoder_layers = 3, learning_rate = 1e-3,
    inverter_learning_rate=1e-1,
    stopping_rounds = 1000, init_type = 'normal', rel_tol = -2, layer_normalization = False, n_embeddings=3, normalize_target=False,
    unnormalized_inputs=['const'])
    train_test(CamelSixHump, 2, forecaster_class=LatentStructuredPICNN, **forecaster_kwargs)


    forecaster_kwargs = dict(
    augment_ctrl_inputs = False, n_hidden_x = 3, n_latent=30, n_out = 1, batch_size = 2500,
    n_epochs = 300, stats_step = 20, n_layers = 3, n_encoder_layers = 3, n_decoder_layers = 3, learning_rate = 1e-3,
    inverter_learning_rate=1e-1,
    ...
    forecaster_kwargs = dict(
    augment_ctrl_inputs = False, n_hidden_x = 3, n_latent=50, n_out = 1, batch_size = 2500,
    n_epochs = 20, stats_step = 20, n_layers = 3, n_encoder_layers = 2, n_decoder_layers = 4, learning_rate = 1e-3,
    inverter_learning_rate=1e-1, inverter_class=optax.adam,
    stopping_rounds = 1000, init_type = 'normal', rel_tol = -2, layer_normalization = False, n_embeddings=10, normalize_target=False,
    unnormalized_inputs=['const'])
