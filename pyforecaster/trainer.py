...
def objective(trial, x: pd.DataFrame, y: pd.DataFrame, model, cv, scoring=nmae, param_space_fun=default_param_space_fun,
    ...
    if hpo_type == 'full':
        cv_results = cross_validate(model, x, y, scoring=scoring, cv=cv, **cv_kwargs)
        ...
        cv_folds = list(cv)
        tr_idx, te_idx = np.random.choice(cv_folds, 1)
        x_tr, x_te, y_tr, y_te = x.loc[tr_idx], x.loc[te_idx], y.loc[tr_idx], y.loc[te_idx]
        model.fit(x_tr, y_tr)
        preds = model.predict(x_te)
        score = scoring(preds, y_te)
...
def hyperpar_optimizer(x, y, model, n_trials, scoring, cv=5, param_space_fun=default_param_space_fun,
